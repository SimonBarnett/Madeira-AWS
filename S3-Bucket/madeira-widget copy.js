(function() {
    ...
    const observer = new IntersectionObserver(entries => {
        console.log('IntersectionObserver triggered, isIntersecting:', entries[0].isIntersecting, 'isIntersecting:', entries[0].isIntersecting, 'hasMore:', hasMore, 'isLoading:', isLoading, 'currentMainCategory:', currentMainCategory);
    ...
    async function detectChromeExtensionSupport() {
      let supportsChromeExt = false;
      let browserInfo = 'Unknown';
      if ('userAgentData' in navigator) {
        const uaData = await navigator.userAgentData.getHighEntropyValues(['brands']);
        const brands = uaData.brands || [];
       
        // Check for 'Chromium' brand, which indicates native Chrome extension support
        if (brands.some(brand => brand.brand === 'Chromium')) {
          supportsChromeExt = true;
          browserInfo = brands.map(b => b.brand).join(', ');
        }
      } else {
        // Fallback for non-Chromium browsers
        supportsChromeExt = false;
        browserInfo = 'Non-Chromium (e.g., Firefox or Safari)';
      }
      console.log(`Supports Chrome extensions: ${supportsChromeExt}`);
      console.log(`Browser brands: ${browserInfo}`);
      return supportsChromeExt;
    }
    ...
    function init() {
        function waitForElements() {
            return new Promise((resolve) => {
                function checkElements(attempt = 1) {
                    ...
                    const sortSelect = document.getElementById('sort-order');
                    if (menuContainer && controlsContainer && partsContainer && categorySelect && filterSelect && sortSelect) {
        ...
        async function initializeWithRetry(attempt = 1) {
            ...
            try {
                console.log(`Initialization attempt ${attempt}/${MAX_RETRIES} at ${new Date().toLocaleTimeString('en-GB', { timeZone: 'Europe/London' })} BST, August 14, 2025`);
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (elementsReady) {
                ...
                const supportsChrome = await detectChromeExtensionSupport();
                if (supportsChrome) {
                  const extensionLink = document.createElement('a');
                  extensionLink.href = 'https://chromewebstore.google.com/detail/club-madeira-affiliate-ex/ilnlmljfigjdlfppgnkffmlpmpdaiegc?utm_source=item-share-cb';
                  extensionLink.target = '_blank';
                  extensionLink.style.position = 'absolute';
                  extensionLink.style.right = '10px';
                  extensionLink.style.top = '0';
                  const extensionImg = document.createElement('img');
                  extensionImg.src = 'https://developer.chrome.com/static/docs/webstore/branding/image/tbyBjqi7Zu733AAKA5n4.png';
                  extensionImg.alt = 'Chrome Web Store Google apps';
                  extensionImg.title = 'Please help our club ranking by installing the extension.';
                  extensionImg.style.width = 'auto';
                  extensionImg.style.height = '30px'; // Adjust size as needed
                  extensionLink.appendChild(extensionImg);
                  controlsContainer.appendChild(extensionLink);
                  console.log('Added Chrome extension image link');
                }
            }
        });
